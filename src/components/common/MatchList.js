...
export class MatchList extends React.Component {
  ...
  render() {
    // pagination
    const {users, page, delete_show} = this.props;
    const per_page = 10;
    const pages = Math.ceil(users.length / per_page);
    const start_offset = (page - 1) * per_page;
    let start_count = 0;
    let final_column = null
    if(!delete_show) {
      final_column=<th>Result</th>
    }
        ...
        <Table bordered hover responsive striped>
          ...
          {users.map((user, index) => {
            if (index >= start_offset && start_count < per_page) {
              start_count++;
              return (
                <MatchListElement key={index} user={user} showDelete={this.showDelete} showLastCol={delete_show}/>
              );
            }
          })}
          </tbody>
